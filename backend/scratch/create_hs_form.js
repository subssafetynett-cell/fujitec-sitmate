...
async function main() {
  ...
  const fields = [
    { id: "site_building", label: "Site/ Building", type: "text", required: true },
    { id: "customer_name", label: "Customer Name", type: "text", required: true },
    { id: "incident_type_header", type: "section_header", subheading: "Incident Type" },
    { 
      id: "incident_types", 
    ...
    { id: "description_header", type: "section_header", subheading: "Describe the health and safety concern [unsafe act / unsafe condition]" },
  ...
  const form = await prisma.form.upsert({
    where: { id: 'health-safety-concern-static-id' }, // We can't really upsert by title easily without a unique constraint, but we can try to find it first
    update: {
      title: formTitle,
      fields: fields,
    },
    create: {
      id: 'health-safety-concern-static-id', // Using a fixed ID for easy reference, but Prisma might require a UUID if it's a string ID. Actually the schema uses @default(uuid()).
      title: formTitle,
      fields: fields,
      createdById: user.id
    }
  }).catch(async (e) => {
      // If the above fails due to ID format, let's just create it with a generated ID
  ...
  console.log("Form created/updated:", form.id);
}
